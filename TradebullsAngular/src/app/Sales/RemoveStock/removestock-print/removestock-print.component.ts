...
@Component({
  selector: 'app-removestock-print',
  templateUrl: './removestock-print.component.html',
  styleUrl: './removestock-print.component.css'
})
export class RemoveStockPrintComponent implements OnInit, AfterContentInit, AfterViewInit {
  ...
  OpenSalesOrder(SID: number) {
    ...
    this.RemoveStockID = SID;
    if (this.RemoveStockID > 0) {
      const model: {
      ...
      this.removeStockService.Get(model).then(
        (response: any) => {
          ...
          if (!response.success) {
            ...
            if (order) {
              ...
              this.VoucherDate = order.RemoveStockDate;
  ...
  GetProductDetails(ProductID: number): RemoveStock_Item[] {
