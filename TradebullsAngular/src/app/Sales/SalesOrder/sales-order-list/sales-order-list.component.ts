...
export class SalesOrderListComponent implements OnInit {
  ...
  displayedColumns = ['Actions', 'SalesOrderID', 'SalesOrderNo', 'SalesOrderDate', 'PartyName', 'PONo', 'PODate', 'Status', 'TotalQty', 'TotalAmount', 'ModifiedByName', 'ModifiedDate'];
  dataSource: MatTableDataSource<any> = new MatTableDataSource<any>();

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;
  @ViewChild("SalesOrderAdd") SalesOrderAdd?: SalesOrderAddComponent;
  @ViewChild("SalesOrderPrint") SalesOrderPrint?: SalesOrderPrintComponent;
  ...
  constructor(private fb: FormBuilder, private router: Router, private salesOrderService: SalesOrderService, private toastr: ToastrService, private el: ElementRef,
  ...
  loadOrders(isReload: boolean = false) {
    ...
    this.salesOrderService.Get(model).then(users => {
  ...
  DeleteRow(item: any) {
    if (confirm("Are you sure you want to delete?")) {
      this.OrderList = this.OrderList.filter(obj => obj.SalesOrderID !== item.SalesOrderID);

      this.salesOrderService.Delete(<number>item.SalesOrderID).subscribe(users => {
        ...
        if (!d.success) {
          ...
          this.toastr.success(item.SalesOrderNo + ' successfully deleted', '', { enableHtml: true, closeButton: true });
  ...
  ExportToExcel() {
    ...
    const FileName: string = "SalesOrder_" + todayDate.getDate() + todayDate.getMonth() + todayDate.getFullYear() + todayDate.getHours() + todayDate.getMinutes() + todayDate.getSeconds() + ".xlsx";

    const filteredData = this.OrderList.map(item => ({
      'Sales Order ID': item.SalesOrderID,
      'Sales Order No': item.SalesOrderNo,
      'Sales Order Date': this.formatDate(item.SalesOrderDate),
  ...
  OpenSalesOrder(SalesOrderID: number) {
    this.IsAddNew = true;
    this.SalesOrderAdd?.OpenSalesOrder(SalesOrderID);
  }
  ...
  PrintRow(item: any) {
    this.IsPrint = true;
    this.SalesOrderPrint?.OpenSalesOrder(item.SalesOrderID);
  }
  PrintClose_Click(){
    this.IsPrint = false;
  }
