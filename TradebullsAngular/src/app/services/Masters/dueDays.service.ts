...
export class DueDaysService {

  private Baseurl:string = environment.APIUrl;
  private APIKey:string = environment.APIKey;
  private ClientCode:string = environment.ClientCode;

  private API_Get = 'Masters/DueDays/Get';
  private API_Delete = 'Masters/DueDays/Delete';
  private API_Create = 'Masters/DueDays/Create';

  constructor(private http: HttpClient) { 
  }

  Get(DueDaysID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("DueDaysID", <any>DueDaysID);
  ...
  Delete(DueDaysID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?DueDaysID="+ DueDaysID, body,{ headers});
  }

  Create(model:DueDaysMaster_AddModel){
