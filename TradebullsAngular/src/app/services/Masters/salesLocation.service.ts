...
export class SalesLocationService {

  private Baseurl:string = environment.APIUrl;
  private APIKey:string = environment.APIKey;
  private ClientCode:string = environment.ClientCode;

  private API_Get = 'Masters/SalesLocation/Get';
  private API_Delete = 'Masters/SalesLocation/Delete';
  private API_Create = 'Masters/SalesLocation/Create';

  constructor(private http: HttpClient) { 
  }

  Get(SalesLocationID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("SalesLocationID", <any>SalesLocationID);
  ...
  Delete(SalesLocationID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?SalesLocationID="+ SalesLocationID, body,{ headers});
  }

  Create(model:SalesLocationMaster_AddModel){
