...
export class BrandService {
  ...
  private API_Get = 'Masters/Brand/Get';
  private API_Delete = 'Masters/Brand/Delete';
  private API_Create = 'Masters/Brand/Create';

  constructor(private http: HttpClient) { 
  }

  Get(BrandID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("BrandID", <any>BrandID);
  ...
  Delete(BrandID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?BrandID="+ BrandID, body,{ headers});
  }

  Create(model:BrandMaster_AddModel){
