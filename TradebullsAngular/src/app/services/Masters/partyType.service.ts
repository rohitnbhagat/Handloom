...
export class PartyTypeService {
  ...
  private API_Get = 'Masters/PartyType/Get';
  private API_Delete = 'Masters/PartyType/Delete';
  private API_Create = 'Masters/PartyType/Create';

  constructor(private http: HttpClient) { 
  }

  Get(PartyTypeID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("PartyTypeID", <any>PartyTypeID);
  ...
  Delete(PartyTypeID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?PartyTypeID="+ PartyTypeID, body,{ headers});
  }

  Create(model:PartyTypeMaster_AddModel){
