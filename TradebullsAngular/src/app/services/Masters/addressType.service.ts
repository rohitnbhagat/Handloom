...
export class AddressTypeService {
  ...
  private API_Get = 'Masters/AddressType/Get';
  private API_Delete = 'Masters/AddressType/Delete';
  private API_Create = 'Masters/AddressType/Create';

  constructor(private http: HttpClient) { 
  }

  Get(AddressTypeID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("AddressTypeID", <any>AddressTypeID);
  ...
  Delete(AddressTypeID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?AddressTypeID="+ AddressTypeID, body,{ headers});
  }

  Create(model:AddressTypeMaster_AddModel){
