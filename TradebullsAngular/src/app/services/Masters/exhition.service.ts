...
export class ExhitionService {

  private Baseurl:string = environment.APIUrl;
  private APIKey:string = environment.APIKey;
  private ClientCode:string = environment.ClientCode;

  private API_Get = 'Masters/Exhition/Get';
  private API_Delete = 'Masters/Exhition/Delete';
  private API_Create = 'Masters/Exhition/Create';

  constructor(private http: HttpClient) { 
  }

  Get(ExhitionID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("ExhitionID", <any>ExhitionID);
  ...
  Delete(ExhitionID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?ExhitionID="+ ExhitionID, body,{ headers});
  }

  Create(model:ExhitionMaster_AddModel){
