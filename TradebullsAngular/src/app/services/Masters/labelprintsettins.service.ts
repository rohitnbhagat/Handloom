...
export class LabelprintsettinsService {
  private Baseurl:string = environment.APIUrl;
  private APIKey:string = environment.APIKey;
  private ClientCode:string = environment.ClientCode;

  private API_Get = 'Masters/LabelPrintSettings/Get';
  private API_Delete = 'Masters/LabelPrintSettings/Delete';
  private API_Create = 'Masters/LabelPrintSettings/Create';

  constructor(private http: HttpClient) { 
  }

  Get(SettingID:number = 0){
    let userSession:any = localStorage.getItem("userSession");
    let context = JSON.parse(<string>userSession);

    let queryParameters = new HttpParams( { encoder: new CustomHttpUrlEncodingCodec() });
    queryParameters = queryParameters.set("SettingID", <any>SettingID);
  ...
  Delete(SettingID:number){
    ...
    return this.http.post(this.Baseurl + this.API_Delete + "?SettingID="+ SettingID, body,{ headers});
  }

  Create(model:Labelprintsettins_AddModel){
