...
import { StateMaster_ViewModel } from '../../../Models/Masters/LocationModel';
...
export class StateListComponent  implements AfterViewInit {
  ...
  displayedColumns = ['StateID','CountryCode','CountryName', 'StateCode', 'StateName','ModifiedByName', 'ModifiedDate', 'Actions'];
  dataSource: MatTableDataSource<StateMaster_ViewModel> = new MatTableDataSource<StateMaster_ViewModel>();

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;

  UserList!: StateMaster_ViewModel[];
  IsCheckboxinit: boolean = false;
  @ViewChild("usercomp") usercomp?: StateAddComponent;
  ...
  loadUsers(isReload: boolean = false) {
    this.isLoading = true;
    this.locationService.GetState(0, 0).then(users => {
  ...
  EditRow(item: any) {
    this.UserList = this.UserList.filter(obj => obj.StateID !== item.StateID);
  }
  DeleteRow(item: any) {
    if (confirm("Are you sure you want to delete?")) {
      this.UserList = this.UserList.filter(obj => obj.StateID !== item.StateID);

      this.locationService.DeleteState(<number>item.StateID).subscribe(users => {
        ...
        if (!d.success) {
          ...
          this.toastr.success(item.StateName + ' successfully deleted', '', { enableHtml: true, closeButton: true });
          this.loadUsers(true);
        }
  ...
  CreateUser(StateID: number = 0) {
    this.usercomp?.OpenModel(StateID);
  }
  ...
  ExportToExcel(){
    ...
    const FileName:string = "State_"+ todayDate.getDate() + todayDate.getMonth() + todayDate.getFullYear() + todayDate.getHours() + todayDate.getMinutes() + todayDate.getSeconds() + ".xlsx";
