...
import { CityMaster_ViewModel } from '../../../Models/Masters/LocationModel';
...
export class CityListComponent  implements AfterViewInit {
  ...
  displayedColumns = ['CityID','CountryCode','CountryName', 'StateCode', 'StateName', 'CityName','ModifiedByName', 'ModifiedDate', 'Actions'];
  dataSource: MatTableDataSource<CityMaster_ViewModel> = new MatTableDataSource<CityMaster_ViewModel>();

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;

  UserList!: CityMaster_ViewModel[];
  IsCheckboxinit: boolean = false;
  @ViewChild("usercomp") usercomp?: CityAddComponent;
  ...
  loadUsers(isReload: boolean = false) {
    this.isLoading = true;
    this.locationService.GetCity(0,0,0).then(users => {
  ...
  EditRow(item: any) {
    this.UserList = this.UserList.filter(obj => obj.CityID !== item.CityID);
  }
  DeleteRow(item: any) {
    if (confirm("Are you sure you want to delete?")) {
      this.UserList = this.UserList.filter(obj => obj.CityID !== item.CityID);

      this.locationService.DeleteCity(<number>item.CityID).subscribe(users => {
        ...
        if (!d.success) {
          ...
          this.toastr.success(item.CityName + ' successfully deleted', '', { enableHtml: true, closeButton: true });
          this.loadUsers(true);
        }
  ...
  CreateUser(CityID: number = 0) {
    this.usercomp?.OpenModel(CityID);
  }
  ...
  ExportToExcel(){
    ...
    const FileName:string = "City_"+ todayDate.getDate() + todayDate.getMonth() + todayDate.getFullYear() + todayDate.getHours() + todayDate.getMinutes() + todayDate.getSeconds() + ".xlsx";
