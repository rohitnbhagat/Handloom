...
namespace HandloomSyncService
{
    public partial class Service1 : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            ...
            Sync_TaxClasses();
            AddHistory(1, startDate, DateTime.Now);
            startDate = DateTime.Now;
            Sync_TaxRates();
            AddHistory(2, startDate, DateTime.Now);
            startDate = DateTime.Now;
            Sync_Customers();
            AddHistory(4, startDate, DateTime.Now);
            startDate = DateTime.Now;
            Sync_Products();
            AddHistory(3, startDate, DateTime.Now);
            startDate = DateTime.Now;
            Sync_Orders();
            AddHistory(5, startDate, DateTime.Now);
            startDate = DateTime.Now;
        }
        ...
        public static void Sync_Customers()
        {
            try {
                WooCommerceHelper wooCommerceHelper = new WooCommerceHelper();
                ...
                if (wooCommerceCustomerSyncResult.success)
                {
                    ...
                    alparameter.Add(new System.Data.SqlClient.SqlParameter("@UserID", 1));
                    clsResult result = dBConnection.executeNonQuery("dbo.PartyMaster_Sync", alparameter);
                    if (result.HasError)
                    {
                        Log("Sync_Customers : result.HasError : " + result.GetException.Message);
                    }
                    else
                        Log("Sync_Customers successfully");
                }
                else
                {
                    Log("wooCommerceCustomerSyncResult.success : " + wooCommerceCustomerSyncResult.Message);
                }
            }
            catch (Exception ex) {
                Log("Sync_Customers : "+ ex.Message);
            }
        }
        public static void Sync_TaxClasses()
        {
            try
            {
                ...
                if (wooCommerceTaxClassSyncResult.success)
                {
                    ...
                    if (result.HasError)
                    {
                        Log("Sync_TaxClasses : result.HasError : " + result.GetException.Message);
                    }
                    else
                        Log("Sync_TaxClasses successfully");
                }
                else
                {
                    Log("wooCommerceTaxClassSyncResult.success : " + wooCommerceTaxClassSyncResult.Message);
                }
            }
            catch (Exception ex)
            {
                Log("Sync_TaxClasses : " + ex.Message);
            }
        }
        public static void Sync_TaxRates()
        {
            try
            {
                ...
                if (wooCommerceTaxRateSyncResult.success)
                {
                    ...
                    if (result.HasError)
                    {
                        Log("Sync_TaxRates : result.HasError : " + result.GetException.Message);
                    }
                    else
                        Log("Sync_TaxRates successfully");
                }
                else
                {
                    Log("wooCommerceTaxRateSyncResult.success : " + wooCommerceTaxRateSyncResult.Message);
                }
            }
            catch (Exception ex)
            {
                Log("Sync_TaxRates : " + ex.Message);
            }
        }
        public static void Sync_Products()
        {
            try
            {
                ...
                if (wooCommerceProductSyncResult.success)
                {
                    ...
                    if (result.HasError)
                    {
                        Log("Sync_Products : result.HasError : " + result.GetException.Message);
                    }
                   else
                        Log("Sync_Products successfully");
                }
                else
                {
                    Log("wooCommerceProductSyncResult.success : " + wooCommerceProductSyncResult.Message);
                }
            }
            catch (Exception ex)
            {
                Log("Sync_Products : " + ex.Message);
            }
        }
        public static void Sync_Orders()
        {
            try
            {
                ...
                if (wooCommerceOrderSyncResult.success)
                {
                    ...
                    if (result.HasError)
                    {
                        Log("Sync_Orders : result.HasError : " + result.GetException.Message);
                    }
                   else
                        Log("Sync_Orders successfully");
                }
                else
                {
                    Log("wooCommerceOrderSyncResult.success : " + wooCommerceOrderSyncResult.Message);
                }
            }
            catch (Exception ex)
            {
                Log("Sync_Orders : " + ex.Message);
            }
        }

        public static void AddHistory(int TableID, DateTime SyncStartDate, DateTime SyncEndDate)
        {
            try
            {
                DBOperation dBConnection = new DBOperation();
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@TableID", TableID));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@SyncStartDate", SyncStartDate));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@SyncEndDate", SyncEndDate));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ContextUserID", 1));
                clsResult result = dBConnection.execute("dbo.SyncTable_Add", alparameter);
                if (result.HasError)
                    Log("AddHistory : result.HasError : " + result.GetException.Message);
                else
                    Log("AddHistory : success");
            }
            catch (Exception ex)
            {
                Log("AddHistory : " + ex.Message);
            }
        }
    }
}
