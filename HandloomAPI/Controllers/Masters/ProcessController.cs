...
using HandloomAPI.Models.Masters.ProcessMaster;
using TrabullsAPI;
...
namespace HandloomAPI.Controllers.Masters
{
    ...
    public class ProcessController : BaseController
    {
        ...
        [HttpPost("Create")]
        public IActionResult Create(ProcessMaster_AddModel model)
        {
            try
            {
                ...
                alparameter.AddRange(TrabullsAPI.Common.GetSQLParameter< ProcessMaster_AddModel>(model));
                clsResult result = dBConnection.execute("dbo.ProcessMaster_Add", alparameter);
                if (result.HasError)
                {
        ...
        [HttpPost("Delete")]
        public IActionResult Delete(long ProcessID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ContextUserID", contextData.UserID));
                clsResult result = dBConnection.executeNonQuery("dbo.ProcessMaster_Delete", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                return Ok(
                    new { success = true, message = "Successfully deleted", data = ProcessID }
                    );
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("Get")]
        public IActionResult Get(Int64 ProcessID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ProcessID", ProcessID));
                clsResult result = dBConnection.execute("dbo.ProcessMaster_Get", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                ...
                {
                    List<ProcessMaster_ViewModel> obj = result.GetDataList<ProcessMaster_ViewModel>();
                    return Ok(new
                    {
                        success = true,
                        message = "success",
