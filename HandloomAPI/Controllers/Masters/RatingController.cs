...
using HandloomAPI.Models.Masters.RatingMaster;
using TrabullsAPI;
...
namespace HandloomAPI.Controllers.Masters
{
    ...
    public class RatingController : BaseController
    {
        ...
        [HttpPost("Create")]
        public IActionResult Create(RatingMaster_AddModel model)
        {
            try
            {
                ...
                alparameter.AddRange(TrabullsAPI.Common.GetSQLParameter< RatingMaster_AddModel>(model));
                clsResult result = dBConnection.execute("dbo.RatingMaster_Add", alparameter);
                if (result.HasError)
                {
        ...
        [HttpPost("Delete")]
        public IActionResult Delete(long RatingID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                DBOperation dBConnection = new DBOperation(_configuration, contextData);
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@RatingID", RatingID));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ContextUserID", contextData.UserID));
                clsResult result = dBConnection.executeNonQuery("dbo.RatingMaster_Delete", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                return Ok(
                    new { success = true, message = "Successfully deleted", data = RatingID }
                    );
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("Get")]
        public IActionResult Get(Int64 RatingID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@RatingID", RatingID));
                clsResult result = dBConnection.execute("dbo.RatingMaster_Get", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                ...
                {
                    List<RatingMaster_ViewModel> obj = result.GetDataList<RatingMaster_ViewModel>();
                    return Ok(new
                    {
                        success = true,
                        message = "success",
