...
using HandloomAPI.Models.Masters.BrandMaster;
using TrabullsAPI;
...
namespace HandloomAPI.Controllers.Masters
{
    ...
    public class BrandController : BaseController
    {
        ...
        [HttpPost("Create")]
        public IActionResult Create(BrandMaster_AddModel model)
        {
            try
            {
                ...
                alparameter.AddRange(TrabullsAPI.Common.GetSQLParameter< BrandMaster_AddModel>(model));
                clsResult result = dBConnection.execute("dbo.BrandMaster_Add", alparameter);
                if (result.HasError)
                {
        ...
        [HttpPost("Delete")]
        public IActionResult Delete(long BrandID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                DBOperation dBConnection = new DBOperation(_configuration, contextData);
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@BrandID", BrandID));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ContextUserID", contextData.UserID));
                clsResult result = dBConnection.executeNonQuery("dbo.BrandMaster_Delete", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                return Ok(
                    new { success = true, message = "Successfully deleted", data = BrandID }
                    );
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("Get")]
        public IActionResult Get(Int64 BrandID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                ArrayList alparameter = new ArrayList();
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@BrandID", BrandID));
                clsResult result = dBConnection.execute("dbo.BrandMaster_Get", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                ...
                {
                    List<BrandMaster_ViewModel> obj = result.GetDataList<BrandMaster_ViewModel>();
                    return Ok(new
                    {
                        success = true,
                        message = "success",
