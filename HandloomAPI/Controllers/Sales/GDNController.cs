...
namespace HandloomAPI.Controllers.Sales
{
    ...
    public class GDNController : BaseController
    {
        ...
        public GDNController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpPost("Delete")]
        public IActionResult Delete(long GDNID)
        {
            try
            {
                ...
                clsResult result = dBConnection.executeNonQuery("dbo.GDN_Delete", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                return Ok(
                    new { success = true, message = "Successfully deleted", data = GDNID }
                    );
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Get")]
        public IActionResult Get(GDN_Filter model)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                clsResult result = dBConnection.execute("dbo.GDN_Get", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
        ...
        public IActionResult GetPendingSalesOrders(SalesOrder_Filter model)
        {
            try
            {
                ...
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@ExhibitionID", model.ExhibitionID));
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@CreditTypeID", model.CreditTypeID));
                clsResult result = dBConnection.execute("dbo.GDN_Get_SalesOrder", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
        ...
        public IActionResult GetPendingSalesOrderDetails(string SalesOrderIDs, long GDNID)
        {
            try
            {
                ContextData contextData = Common.GetContextData(Request);
                ...
                alparameter.Add(new System.Data.SqlClient.SqlParameter("@GDNID", GDNID));
                clsResult result = dBConnection.execute("dbo.GDN_Get_SalesOrder_Items", alparameter);
                if (result.HasError)
                {
                    return new NotFoundObjectResult(new APIResult() { success = false, message = result.GetException.Message, data = null });
                }
                ...
                {
                    List<GDN_SalesOrder_Item_ViewModel> obj = result.GetDataList<GDN_SalesOrder_Item_ViewModel>();
                    List<GDN_SalesOrder_Tax_ViewModel> Taxes = new List<GDN_SalesOrder_Tax_ViewModel>();

                    if (result.ResultDataSet.Tables.Count > 1)
                    {
                        List<GDN_SalesOrder_Item_Attribute_ViewModel> lstAttribute = clsConvert.ConvertDataTable<GDN_SalesOrder_Item_Attribute_ViewModel>(result.ResultDataSet.Tables[1]);
                        if (lstAttribute.Count > 0)
                        {
                            foreach (GDN_SalesOrder_Item_ViewModel item in obj)
                            {
                    ...
                    if (result.ResultDataSet.Tables.Count > 2)
                    {
                        List<GDN_SalesOrder_Item_Tax_ViewModel> lstTaxes = clsConvert.ConvertDataTable<GDN_SalesOrder_Item_Tax_ViewModel>(result.ResultDataSet.Tables[2]);
                        if (lstTaxes.Count > 0)
                        {
                            foreach (GDN_SalesOrder_Item_ViewModel item in obj)
                            {
                    ...
                    if (result.ResultDataSet.Tables.Count > 3)
                    {
                        Taxes = clsConvert.ConvertDataTable<GDN_SalesOrder_Tax_ViewModel>(result.ResultDataSet.Tables[3]);
                    }
